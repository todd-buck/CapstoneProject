...
namespace DDRInventory.Models
{
    public class Database : IDisposable
    {
        ...
        public Database()
        {
            _connection = new SQLiteConnection($"Data Source={DATABASE_PATH}{Path.DirectorySeparatorChar}{DATABASE_NAME}; Version = 3; New = True; Compress = True; ");
            // Open the connection:
            try
            {
                _connection.Open();
        ...
        public void Init()
        {
            File.Copy($"{DATABASE_PATH}{Path.DirectorySeparatorChar}DefaultDatabase.db", $"{DATABASE_PATH}{Path.DirectorySeparatorChar}{DATABASE_NAME}", true);
            CheckVersion();
        ...
        public static async void RefreshAsync()
        {
            while (true)
            {
                ...
                Console.WriteLine($"DEFAULT DATABASE PATH: {DATABASE_PATH}{Path.DirectorySeparatorChar}DefaultDatabase.db");
                Console.WriteLine($"WORKING DATABASE PATH: {DATABASE_PATH}{Path.DirectorySeparatorChar}{DATABASE_NAME}");
                File.Copy($"{DATABASE_PATH}{Path.DirectorySeparatorChar}DefaultDatabase.db", $"{DATABASE_PATH}{Path.DirectorySeparatorChar}{DATABASE_NAME}", true);
